...
def _add_issue(issues_list, severity, issue_type, location, description, thread_group="N/A", element_name="N/A"):
    ...
    print(f"Validation: {THIS_VALIDATION_OPTION_NAME}")
    print(f"Type: {issue_type}")
    print(f"Location: {location}")
    print(f"Element Name: {element_name}")
    print(f"Description: {description}")
    print(f"Thread Group/Context: {thread_group}")  # Updated for clarity
    print(f"----------------------\n")
    issues_list.append({
        'severity': severity,
...
def _get_property_value(element, prop_name):
    ...
    # Uncomment the print below if you want to see detailed property retrieval debug
...
def _get_bool_property_value(element, prop_name):
    ...
    if prop_element is not None:
        # Uncomment the print below if you want to see detailed bool property retrieval debug
...
def _validate_common_extractor_rules(extractor_element, issues, container_context):
    ...
    print(
        f"DEBUG: Validating common rules for extractor: '{element_name}' (Tag: {extractor_element.tag}) in Context: {container_context}")

    # --- CORRECTED PROPERTY NAMES FOR ALL EXTRACTORS ---
    variable_name = ""
    default_value = ""
    match_no_str = ""
    scope = ""
    ...
    if extractor_element.tag == 'JSONPostProcessor':
        ...
        # CORRECTED: 'matchNumber' instead of 'match_number'
        match_no_str = _get_property_value(extractor_element, 'XPathExtractor.matchNumber')
        ...
        # CORRECTED: 'default' instead of 'default_value'
        default_value = _get_property_value(extractor_element, 'BoundaryExtractor.default')
    ...
    print(
        f"DEBUG:   Common Props -> Var Name: '{variable_name}', Default: '{default_value}', Match No: '{match_no_str}', Scope: '{scope}'")
    ...
    try:
        if match_no_str: # Only try to convert if string is not empty
            ...
            # No issue for match_no = 0 (random) or 1 (first match)
            # Only report if it's not a valid integer or empty
        else:
...
def _validate_boundary_extractor(extractor_element, issues, container_context):
    ...
    # CORRECTED: 'lboundary' instead of 'leftBoundary'
    left_boundary = _get_property_value(extractor_element, 'BoundaryExtractor.lboundary')
    # CORRECTED: 'rboundary' instead of 'rightBoundary'
...
def analyze_jmeter_script(root_element, enabled_validations):
    ...
    current_thread_group_context = "Global/Unassigned"  # Default context if extractor is not under a TG/TF

    # Iterate through ALL elements in the JMX tree
    for element in root_element.iter():
        element_tag = element.tag
        element_name = _get_element_name(element)

        # Update current thread group/context if a new relevant container is found
        ...
        # Check if the current element is an extractor
        if element_tag in ['RegexExtractor', 'JSONPostProcessor', 'XPathExtractor', 'CssSelectorExtractor',
                           'BoundaryExtractor']:
            print(
                f"DEBUG:   Found extractor: Tag='{element_tag}', Name='{element_name}' within context '{current_thread_group_context}'")

            # Call specific validation functions, passing the current context
    ...
    print(
        f"--- DEBUG: analyze_jmeter_script for {THIS_VALIDATION_OPTION_NAME} finished. Found {len(module_issues)} issues. ---\n")
    return module_issues
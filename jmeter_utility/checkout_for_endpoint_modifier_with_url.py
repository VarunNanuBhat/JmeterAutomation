...
class CheckoutPageForEndpointModifierWithURL(ttk.Frame):
    def __init__(self, parent):
        super().__init__(parent, padding=20)
        self.parent = parent
        self.endpoints_to_modify = []
        self.action = ""
        ...
        back_button = ttk.Button(button_frame, text="🔙 Back", bootstyle="secondary",
                                 command=self.go_back_to_endpoint_modifier_with_url)
    ...
    def display_changes(self, endpoints, action):
        """Display the changes queued for confirmation."""
        self.endpoints_to_modify = endpoints  # Store the endpoints
        ...
        # Display action and endpoints
        ttk.Label(self.preview_frame, text=f"Action: {action.capitalize()}", font=("Arial", 14, "bold")).pack(
            anchor="w", padx=20, pady=10)
        for idx, endpoint in enumerate(endpoints, start=1):
            ttk.Label(self.preview_frame, text=f"{idx}. {endpoint}", font=("Arial", 12)).pack(anchor="w", padx=20,
                                                                                              pady=5)

    def go_back_to_endpoint_modifier_with_url(self):
        """Return to the Endpoint Modifier with URL page."""
    ...
    def confirm_changes(self):
        ...
        if not self.endpoints_to_modify:
            self.status_label.config(text="❌ No endpoint suffixes provided!", bootstyle="danger")
            return

        error_messages = []  # Collect errors per file
        success = False  # Track if any endpoint was modified successfully

        for file_path in uploaded_files:
            try:
                modifier = JMXModifier(file_path)
                for endpoint in self.endpoints_to_modify:
                    result = modifier.update_endpoints(endpoint, self.action)

                    if not result:  # If no endpoints were modified, return a warning
                        error_messages.append(f"⚠ No matching endpoints found for '{endpoint}'.")
                    else:
    ...
    def go_back_to_file_upload(self):
        """Reset status label and go back to the file upload page."""
        self.status_label.config(text="")

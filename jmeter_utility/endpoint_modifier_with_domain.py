...
class EndpointActionPageForDomain(ttk.Frame):
    def __init__(self, parent):
        ...
        # Title
        title_label = ttk.Label(self, text="Endpoint Modifier", font=("Arial", 22, "bold"))
        title_label.grid(row=0, column=0, columnspan=2, pady=20)

        # Text Input
        text_label = ttk.Label(self, text="Enter domain to match endpoints:", font=("Arial", 14))
    ...
    def apply_action(self):
        ...
        if not text:
            self.status_label.config(text="Please enter text to match endpoints.", bootstyle="danger")
            return
        ...
        try:
            ...
            for file_path in self.parent.file_upload_page.uploaded_file_paths:
                ...
                success = modifier.update_domain_endpoints(text, action)
                if not success:
                    error_message = f"No matching endpoints found in file: {file_path}"
                    break
                # Save changes
            ...
            if error_message:
                ...
                self.status_label.config(text=f"Endpoints {action}d successfully!", bootstyle="success")
